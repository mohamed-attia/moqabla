...
const InterviewEvaluation: React.FC<Props> = ({ registration, onComplete, onCancel }) => {
        ...
        {!isReviewing ? (
          ...
          <div className="flex-grow overflow-y-auto p-6 md:p-10 custom-scrollbar">
            <div className="bg-gray-50 p-6 md:p-12 rounded-[4rem] border-4 border-dashed border-gray-200 relative">
              ...
              <div className="bg-white rounded-[3rem] p-8 md:p-12 shadow-2xl border border-gray-100 prose prose-teal max-w-none dir-rtl text-right">
                <textarea 
                  value={finalReport || ''} 
              ...
              <>
                ...
                {activeStep < sections.length - 1 ? (
                  <Button onClick={() => setActiveStep(activeStep + 1)} className="flex-1 md:flex-none rounded-2xl px-12 gap-3 group">
                    القسم التالي <ChevronLeft className="w-5 h-5 group-hover:-translate-x-1 transition-transform" />
                  </Button>
                ) : (
