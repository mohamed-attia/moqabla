      ...
      <div className="container mx-auto px-4 md:px-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 cursor-pointer" onClick={() => handleNavigation('home')}>
            ...
            <span className={`text-2xl font-bold ${scrolled || !isHome ? 'text-primary' : 'text-primary md:text-white'}`}>
              مقابلة
            </span>
          </div>
          ...
          >
            {isOpen ? <X className="w-8 h-8 text-primary" /> : <Menu className={`w-8 h-8 ${scrolled || !isHome ? 'text-primary' : 'text-primary'}`} />}
          </button>
        </div>
      </div>
        ...
        <div className="flex flex-col p-4 space-y-4">
          {navItems.map((item) => (
            <button
              key={item.id}
              onClick={() => handleNavigation(item.id)}
              className="text-right text-gray-700 font-medium text-lg py-2 border-b border-gray-50 hover:text-accent hover:bg-gray-50 px-2 rounded transition-colors"
            >
              {item.label}
            </button>
          ))}
