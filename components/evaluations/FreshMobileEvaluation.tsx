...
import { 
  ...
  X, Award, Zap, AlertTriangle, Info, Smartphone, Tablet
} from 'lucide-react';
import Button from '../Button';
import { AIAgent } from '../../lib/ai-agent';
import { FRESH_MOBILE_EVALUATION_TEMPLATE } from '../../lib/evaluation-templates';
...
const FreshMobileEvaluation: React.FC<Props> = ({ registration, onComplete, onCancel, interviewerName }) => {
  ...
  const [sections, setSections] = useState<EvaluationSection[]>(JSON.parse(JSON.stringify(FRESH_MOBILE_EVALUATION_TEMPLATE)));
  ...
  const handleAiSuggest = async (sectionIdx: number, itemIdx: number) => {
    ...
    try {
      const suggestion = await AIAgent.suggestNote(
          item.skill, 
          item.score, 
          'Fresh Mobile Engineer (Android/iOS focus)'
      );
    ...
    try {
      const report = await AIAgent.generateFinalReport(
        ...
        { 
          ...
          decision: totalScore >= 70 ? 'Ready for Fresh Mobile Developer' : totalScore >= 55 ? 'Almost Ready' : 'Not Ready Yet'
        }
      );
    ...
    <div className="fixed inset-0 z-[100] bg-slate-900/90 backdrop-blur-xl flex items-center justify-center p-0 md:p-4 overflow-y-auto">
      <div className="bg-white w-full max-w-6xl md:h-[90vh] md:rounded-[3rem] shadow-2xl flex flex-col animate-in zoom-in-95 duration-300 overflow-hidden relative">
        ...
        <button 
          ...
          className="absolute left-6 top-6 z-[110] bg-indigo-600 hover:bg-indigo-700 text-white p-3 rounded-2xl border border-white/20 transition-all flex items-center gap-2 group shadow-xl"
        >
          <Info className="w-5 h-5 group-hover:rotate-12 transition-transform" />
          <span className="text-xs font-black">أسئلة Mobile مقترحة</span>
        </button>
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-6">
            <div className="flex items-center gap-5">
              ...
              <div className="text-right">
                <h2 className="text-xl md:text-2xl font-black">تقييم موبايل: {registration.fullName}</h2>
                <div className="flex items-center gap-2 text-accent/90 text-sm font-bold mt-1">
                  <Award className="w-4 h-4" />
                  <span>Fresh Mobile Assessment Flow</span>
                </div>
              </div>
            </div>
        ...
        <div className="flex flex-grow overflow-hidden">
                    ...
                    <div key={i} className="animate-in fade-in slide-in-from-bottom-4" style={{ animationDelay: `${i * 100}ms` }}>
                      <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-6">
                        <div className="space-y-1 flex-grow">
                          ...
                          <p className="text-xs text-gray-400 mr-9">قيم مهارات المرشح في {item.skill} لمستوى Fresh Mobile.</p>
                        </div>
                      ...
                      <div className="relative group mr-9">
                        ...
                        <textarea
                          ...
                          placeholder="أضف ملاحظاتك الفنية لمرشح الموبايل هنا..."
              ...
              <div className="bg-gray-50 p-6 md:p-12 rounded-[3rem] border-2 border-dashed border-gray-200">
                <div className="flex items-center justify-between mb-10 flex-wrap gap-4 text-right">
                  <div className="flex items-center gap-4">
                    ...
                    <h3 className="text-2xl font-black text-gray-800">مراجعة تقرير Fresh Mobile</h3>
                  </div>
                  ...
                  <div className="mb-6 p-4 bg-emerald-50 rounded-2xl border border-emerald-100 flex justify-between items-center no-pdf">
                     ...
                     <div className="flex flex-col items-end">
                       ...
                       <span className={`text-[10px] font-bold ${calculateTotalScore() >= 70 ? 'text-emerald-500' : calculateTotalScore() >= 55 ? 'text-amber-500' : 'text-rose-500'}`}>
                         {calculateTotalScore() >= 70 ? 'Ready for Fresh Mobile' : calculateTotalScore() >= 55 ? 'Almost Ready' : 'Not Ready Yet'}
                       </span>
                     </div>
                  </div>
                  <textarea 
          ...
          {/* Sidebar: Mobile Specific Question Map */}
          {showQuestions && (
            <div className="w-80 bg-gray-50 border-r border-gray-100 overflow-y-auto p-6 animate-in slide-in-from-left duration-300 text-right">
              <div className="flex items-center justify-between mb-8">
                ...
                <h4 className="font-black text-gray-900 flex items-center gap-2">
                   <Smartphone className="w-4 h-4 text-accent" /> بنك الأسئلة
                </h4>
              </div>
              ...
              <div className="space-y-8">
                <section>
                  <h5 className="text-[10px] font-black text-indigo-600 uppercase tracking-widest mb-3 border-b border-indigo-100 pb-1 flex items-center gap-2">
                    <Tablet className="w-3 h-3" /> Core Mobile
                  </h5>
                  <ul className="space-y-3 text-xs font-medium text-gray-600 list-disc list-inside leading-relaxed">
                    <li>What is a screen/view in a mobile app?</li>
                    <li>How does interaction trigger logic?</li>
                    <li>App behavior in background mode?</li>
                    <li>Handling empty data on a screen?</li>
                  </ul>
                </section>
                ...
                <section>
                  ...
                  <ul className="space-y-3 text-xs font-medium text-gray-600 list-disc list-inside leading-relaxed">
                    <li>Activity vs Fragment difference.</li>
                    <li>Android Lifecycle (High level).</li>
                    <li>What is RecyclerView for?</li>
                    <li>Passing data (Intent/Bundle).</li>
                    <li>ViewModel concept?</li>
                  </ul>
                </section>
                ...
                <section>
                  ...
                  <ul className="space-y-3 text-xs font-medium text-gray-600 list-disc list-inside leading-relaxed">
                    <li>What is a ViewController?</li>
                    <li>iOS App Lifecycle basics.</li>
                    <li>Storyboard vs SwiftUI?</li>
                    <li>How to navigate screens?</li>
                    <li>Optional in Swift?</li>
                  </ul>
                </section>

                <section>
                  <h5 className="text-[10px] font-black text-amber-600 uppercase tracking-widest mb-3 border-b border-amber-100 pb-1 flex items-center gap-2">
                     UI Thinking
                  </h5>
                  <ul className="space-y-3 text-xs font-medium text-gray-600 list-disc list-inside leading-relaxed">
                    <li>Design a screen for a User List.</li>
                    <li>Handling slow API / loading state.</li>
                    <li>How to show user errors?</li>
                  </ul>
                </section>
                
                <div className="bg-indigo-50 p-4 rounded-2xl border border-indigo-100">
                  <h6 className="text-[10px] font-black text-indigo-900 flex items-center gap-2 mb-1">
                    <Zap className="w-3 h-3" /> نصيحة للمحاور
                  </h6>
                  <p className="text-[10px] leading-relaxed text-indigo-700 italic">
                    Fresh Mobile != Framework expert.
                    قيم الأساسيات، طريقة التفكير، والرغبة الصادقة في التعلم.
                  </p>
                </div>
              </div>
            </div>
...
export default FreshMobileEvaluation;
