...
import { 
  ...
  X, Award, Zap, AlertTriangle, Info, Palette, Layers, MousePointer
} from 'lucide-react';
import Button from '../Button';
import { AIAgent } from '../../lib/ai-agent';
import { FRESH_UX_EVALUATION_TEMPLATE } from '../../lib/evaluation-templates';
...
const FreshUXEvaluation: React.FC<Props> = ({ registration, onComplete, onCancel, interviewerName }) => {
  ...
  const [sections, setSections] = useState<EvaluationSection[]>(JSON.parse(JSON.stringify(FRESH_UX_EVALUATION_TEMPLATE)));
  ...
  const handleAiSuggest = async (sectionIdx: number, itemIdx: number) => {
    ...
    try {
      const suggestion = await AIAgent.suggestNote(
          item.skill, 
          item.score, 
          'Fresh UX Designer (Design Thinking & Research focus)'
      );
    ...
    try {
      const report = await AIAgent.generateFinalReport(
        ...
        { 
          ...
          decision: totalScore >= 70 ? 'Ready for Fresh UX Designer' : totalScore >= 55 ? 'Almost Ready' : 'Not Ready Yet'
        }
      );
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-slate-900/90 backdrop-blur-xl flex items-center justify-center p-0 md:p-4 overflow-y-auto">
      <div className="bg-white w-full max-w-6xl md:h-[90vh] md:rounded-[3rem] shadow-2xl flex flex-col animate-in zoom-in-95 duration-300 overflow-hidden relative">
        ...
        {/* Designer Reference Sidebar Trigger */}
        <button 
          onClick={() => setShowQuestions(!showQuestions)}
          className="absolute left-6 top-6 z-[110] bg-teal-600 hover:bg-teal-700 text-white p-3 rounded-2xl border border-white/20 transition-all flex items-center gap-2 group shadow-xl"
        >
          <Info className="w-5 h-5 group-hover:rotate-12 transition-transform" />
          <span className="text-xs font-black">مرجع الأسئلة (UX)</span>
        </button>
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-6">
            <div className="flex items-center gap-5">
              ...
              <div className="text-right">
                <h2 className="text-xl md:text-2xl font-black">تقييم تجربة المستخدم: {registration.fullName}</h2>
                <div className="flex items-center gap-2 text-accent/90 text-sm font-bold mt-1">
                  <Award className="w-4 h-4" />
                  <span>Fresh UX Design Assessment Flow</span>
                </div>
              </div>
            </div>
                    ...
                    <div key={i} className="animate-in fade-in slide-in-from-bottom-4" style={{ animationDelay: `${i * 100}ms` }}>
                      <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-6">
                        <div className="space-y-1 flex-grow">
                          ...
                          <p className="text-xs text-gray-400 mr-9">قيم مهارات المصمم في {item.skill} لمستوى Fresh UX.</p>
                        </div>
                      ...
                      <div className="relative group mr-9">
                        ...
                        <textarea
                          ...
                          placeholder="أضف ملاحظاتك التحليلية لمرشح الـ UX هنا..."
                        ...
                        >
                          {aiGenerating === `${activeStep}-${i}` ? (
                            ...
                            <>
                              ...
                              <span className="text-[10px] font-black hidden group-hover/ai:block">تحليل ذكي</span>
                            </>
                          )}
                        </button>
                      </div>
              ...
              <div className="bg-gray-50 p-6 md:p-12 rounded-[3rem] border-2 border-dashed border-gray-200">
                <div className="flex items-center justify-between mb-10 flex-wrap gap-4 text-right">
                  <div className="flex items-center gap-4">
                    ...
                    <h3 className="text-2xl font-black text-gray-800">مراجعة تقرير Fresh UX Designer</h3>
                  </div>
                ...
                <div className="bg-white rounded-[2rem] p-6 md:p-10 shadow-sm border border-gray-100 prose prose-teal max-w-none dir-rtl text-right">
                  <div className="mb-6 p-4 bg-emerald-50 rounded-2xl border border-emerald-100 flex justify-between items-center no-pdf">
                     <span className="font-bold text-emerald-800">الدرجة النهائية المستحقة:</span>
                     <div className="flex flex-col items-end">
                       <span className="text-3xl font-black text-emerald-600">{calculateTotalScore()}%</span>
                       <span className={`text-[10px] font-bold ${calculateTotalScore() >= 70 ? 'text-emerald-500' : calculateTotalScore() >= 55 ? 'text-amber-500' : 'text-rose-500'}`}>
                         {calculateTotalScore() >= 70 ? 'Ready for Fresh UX Designer' : calculateTotalScore() >= 55 ? 'Almost Ready' : 'Not Ready Yet'}
                       </span>
                     </div>
                  </div>
                  <textarea 
            ...
            <div className="w-80 bg-gray-50 border-r border-gray-100 overflow-y-auto p-6 animate-in slide-in-from-left duration-300 text-right">
              <div className="flex items-center justify-between mb-8">
                ...
                <h4 className="font-black text-gray-900 flex items-center gap-2">
                   <Palette className="w-4 h-4 text-accent" /> بنك الأسئلة
                </h4>
              </div>

              <div className="space-y-8">
                <section>
                  <h5 className="text-[10px] font-black text-teal-600 uppercase tracking-widest mb-3 border-b border-teal-100 pb-1 flex items-center gap-2">
                    <Brain className="w-3 h-3" /> UX Fundamentals
                  </h5>
                  <ul className="space-y-3 text-xs font-medium text-gray-600 list-disc list-inside leading-relaxed">
                    <li>What is the difference between UX and UI?</li>
                    <li>Why is user-centered design important?</li>
                    <li>How do you decide if a design is "good"?</li>
                  </ul>
                </section>

                <section>
                  <h5 className="text-[10px] font-black text-blue-600 uppercase tracking-widest mb-3 border-b border-blue-100 pb-1 flex items-center gap-2">
                     User Research
                  </h5>
                  <ul className="space-y-3 text-xs font-medium text-gray-600 list-disc list-inside leading-relaxed">
                    <li>Who is the target user for this product?</li>
                    <li>How would you discover user pain points?</li>
                    <li>What questions would you ask users?</li>
                  </ul>
                </section>

                <section>
                  <h5 className="text-[10px] font-black text-indigo-600 uppercase tracking-widest mb-3 border-b border-indigo-100 pb-1 flex items-center gap-2">
                    <Layers className="w-3 h-3" /> Wireframing & Flow
                  </h5>
                  <p className="text-[10px] text-gray-400 mb-2 italic">Design task: Login → Home → Profile</p>
                  <ul className="space-y-3 text-xs font-medium text-gray-600 list-disc list-inside leading-relaxed">
                    <li>What happens if there is no data? (Empty state).</li>
                    <li>How do users navigate back?</li>
                    <li>Visual hierarchy explanation.</li>
                  </ul>
                </section>

                <section>
                  <h5 className="text-[10px] font-black text-rose-500 uppercase tracking-widest mb-3 border-b border-rose-100 pb-1 flex items-center gap-2">
                    <MousePointer className="w-3 h-3" /> Tools & Collab
                  </h5>
                  <ul className="space-y-3 text-xs font-medium text-gray-600 list-disc list-inside leading-relaxed">
                    <li>Which design tools have you used?</li>
                    <li>How do you hand off to developers?</li>
                    <li>Ensuring consistency across screens?</li>
                  </ul>
                </section>
                
                <div className="bg-teal-50 p-4 rounded-2xl border border-teal-100">
                  <h6 className="text-[10px] font-black text-teal-900 flex items-center gap-2 mb-1">
                    <Zap className="w-3 h-3" /> نصيحة للمحاور
                  </h6>
                  <p className="text-[10px] leading-relaxed text-teal-700 italic">
                    Fresh UX != Visual Artist.
                    قيم طريقة التفكير (Process)، التعاطف مع المستخدم (Empathy)، ووضوح التفسير.
                  </p>
                </div>
              </div>
            </div>
...
export default FreshUXEvaluation;
