...
import { 
  ...
  X, Award, Zap, AlertTriangle, Info, Smartphone, Tablet, Coffee
} from 'lucide-react';
import Button from '../Button';
import { AIAgent } from '../../lib/ai-agent';
import { JUNIOR_MOBILE_EVALUATION_TEMPLATE } from '../../lib/evaluation-templates';
...
const JuniorMobileEvaluation: React.FC<Props> = ({ registration, onComplete, onCancel, interviewerName }) => {
  ...
  const [sections, setSections] = useState<EvaluationSection[]>(JSON.parse(JSON.stringify(JUNIOR_MOBILE_EVALUATION_TEMPLATE)));
  ...
  const handleAiSuggest = async (sectionIdx: number, itemIdx: number) => {
    ...
    try {
      const suggestion = await AIAgent.suggestNote(
          item.skill, 
          item.score, 
          'Junior Mobile Engineer (Android/iOS focus)'
      );
    ...
    try {
      const report = await AIAgent.generateFinalReport(
        ...
        { 
          ...
          decision: totalScore >= 75 ? 'Ready for Junior Mobile Developer' : totalScore >= 60 ? 'Needs More Practice' : 'Not Ready Yet'
        }
      );
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-slate-900/90 backdrop-blur-xl flex items-center justify-center p-0 md:p-4 overflow-y-auto">
      <div className="bg-white w-full max-w-6xl md:h-[90vh] md:rounded-[3rem] shadow-2xl flex flex-col animate-in zoom-in-95 duration-300 overflow-hidden relative">
        ...
        {/* Sidebar Reference Button */}
        <button 
          onClick={() => setShowQuestions(!showQuestions)}
          className="absolute left-6 top-6 z-[110] bg-primary text-white p-3 rounded-2xl border border-white/10 transition-all flex items-center gap-2 group shadow-xl hover:bg-slate-800"
        >
          <Info className="w-5 h-5 group-hover:rotate-12 transition-transform" />
          <span className="text-xs font-black">أسئلة Junior Mobile</span>
        </button>
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-6">
            <div className="flex items-center gap-5">
              ...
              <div className="text-right">
                <h2 className="text-xl md:text-2xl font-black">تقييم جونيور موبايل: {registration.fullName}</h2>
                <div className="flex items-center gap-2 text-accent/90 text-sm font-bold mt-1">
                  <Award className="w-4 h-4" />
                  <span>Junior Mobile Assessment Flow</span>
                </div>
              </div>
            </div>
        ...
        <div className="flex flex-grow overflow-hidden">
                    ...
                    <div key={i} className="animate-in fade-in slide-in-from-bottom-4" style={{ animationDelay: `${i * 100}ms` }}>
                      ...
                      <div className="relative group mr-9">
                        ...
                        <textarea
                          ...
                          placeholder="أضف ملاحظاتك الفنية لمرشح الموبايل الجونيور هنا..."
              ...
              <div className="bg-gray-50 p-6 md:p-12 rounded-[3rem] border-2 border-dashed border-gray-200">
                <div className="flex items-center justify-between mb-10 flex-wrap gap-4 text-right">
                  <div className="flex items-center gap-4">
                    ...
                    <h3 className="text-2xl font-black text-gray-800">مراجعة تقرير Junior Mobile</h3>
                  </div>
                  ...
                  <div className="mb-6 p-4 bg-emerald-50 rounded-2xl border border-emerald-100 flex justify-between items-center no-pdf">
                     ...
                     <div className="flex flex-col items-end">
                       ...
                       <span className={`text-[10px] font-bold ${calculateTotalScore() >= 75 ? 'text-emerald-500' : calculateTotalScore() >= 60 ? 'text-amber-500' : 'text-rose-500'}`}>
                         {calculateTotalScore() >= 75 ? 'Ready for Junior Mobile' : calculateTotalScore() >= 60 ? 'Needs More Practice' : 'Not Ready Yet'}
                       </span>
                     </div>
                  </div>
                  <textarea 
          ...
          {/* Sidebar: Junior Mobile Specific Question Map */}
          {showQuestions && (
            <div className="w-80 bg-gray-50 border-r border-gray-100 overflow-y-auto p-6 animate-in slide-in-from-left duration-300 text-right">
              <div className="flex items-center justify-between mb-8">
                ...
                <h4 className="font-black text-gray-900 flex items-center gap-2">
                   <Coffee className="w-4 h-4 text-accent" /> بنك الأسئلة للمحاور
                </h4>
              </div>
              ...
              <div className="space-y-8">
                <section>
                  ...
                  <ul className="space-y-3 text-xs font-medium text-gray-600 list-disc list-inside leading-relaxed">
                    <li>OOP concepts مع مثال من الموبايل.</li>
                    <li>إزاي تكتب كود قابل لإعادة الاستخدام؟</li>
                    <li>إزاي بتتعامل مع errors في الـ App؟</li>
                  </ul>
                </section>
                ...
                <section>
                  ...
                  <ul className="space-y-3 text-xs font-medium text-gray-600 list-disc list-inside leading-relaxed">
                    <li>صمم flow لشاشتين (List → Details).</li>
                    <li>التعامل مع loading, empty, error states.</li>
                    <li>لو التصميم اتغير، هتعدل فين؟ (Separation of concerns).</li>
                  </ul>
                </section>
                ...
                <section>
                  ...
                  <ul className="space-y-3 text-xs font-medium text-gray-600 list-disc list-inside leading-relaxed">
                    <li>Activity vs Fragment lifecycle.</li>
                    <li>ViewModel + lifecycle فائدته.</li>
                ...
                <section>
                  ...
                  <ul className="space-y-3 text-xs font-medium text-gray-600 list-disc list-inside leading-relaxed">
                    <li>ViewController lifecycle detail.</li>
                    <li>App states (active/background).</li>
                    <li>Memory management (basic idea).</li>
                  </ul>
                </section>
                ...
                <div className="bg-amber-50 p-4 rounded-2xl border border-amber-100">
                  ...
                  <p className="text-[10px] leading-relaxed text-amber-700 italic">
                    ...
                    قيم الاستقلالية، التفكير في UX، وفهم الـ Lifecycle وجودة الكود.
                  </p>
                </div>
              </div>
            </div>
...
export default JuniorMobileEvaluation;
