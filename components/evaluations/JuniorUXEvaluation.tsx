...
import { 
  ...
  X, Award, Zap, AlertTriangle, Info, Palette, Search, PenTool, GitMerge, MessageSquare
} from 'lucide-react';
import Button from '../Button';
import { AIAgent } from '../../lib/ai-agent';
import { JUNIOR_UX_EVALUATION_TEMPLATE } from '../../lib/evaluation-templates';
...
const JuniorUXEvaluation: React.FC<Props> = ({ registration, onComplete, onCancel, interviewerName }) => {
  ...
  const [sections, setSections] = useState<EvaluationSection[]>(JSON.parse(JSON.stringify(JUNIOR_UX_EVALUATION_TEMPLATE)));
  ...
  const handleAiSuggest = async (sectionIdx: number, itemIdx: number) => {
    ...
    try {
      const suggestion = await AIAgent.suggestNote(
          item.skill, 
          item.score, 
          'Junior UX Designer (Reasoning & Ownership focus)'
      );
    ...
    try {
      const report = await AIAgent.generateFinalReport(
        ...
        { 
          ...
          decision: totalScore >= 75 ? 'Ready for Junior UX Designer' : totalScore >= 60 ? 'Needs More Experience' : 'Not Ready Yet'
        }
      );
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-slate-900/90 backdrop-blur-xl flex items-center justify-center p-0 md:p-4 overflow-y-auto">
      <div className="bg-white w-full max-w-6xl md:h-[90vh] md:rounded-[3rem] shadow-2xl flex flex-col animate-in zoom-in-95 duration-300 overflow-hidden relative">
        ...
        {/* Designer Sidebar Trigger */}
        <button 
          onClick={() => setShowQuestions(!showQuestions)}
          className="absolute left-6 top-6 z-[110] bg-violet-600 hover:bg-violet-700 text-white p-3 rounded-2xl border border-white/20 transition-all flex items-center gap-2 group shadow-xl"
        >
          <Info className="w-5 h-5 group-hover:rotate-12 transition-transform" />
          <span className="text-xs font-black">أسئلة Junior UX</span>
        </button>
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-6">
            <div className="flex items-center gap-5">
              ...
              <div className="text-right">
                <h2 className="text-xl md:text-2xl font-black">تقييم جونيور UX: {registration.fullName}</h2>
                <div className="flex items-center gap-2 text-accent/90 text-sm font-bold mt-1">
                  <Award className="w-4 h-4" />
                  <span>Junior UX Design Assessment Flow</span>
                </div>
              </div>
            </div>
        ...
        <div className="flex flex-grow overflow-hidden">
                    ...
                    <div key={i} className="animate-in fade-in slide-in-from-bottom-4" style={{ animationDelay: `${i * 100}ms` }}>
                      ...
                      <div className="relative group mr-9">
                        ...
                        <textarea
                          ...
                          placeholder="أضف ملاحظاتك التحليلية لمرشح الجونيور هنا..."
              ...
              <div className="bg-gray-50 p-6 md:p-12 rounded-[3rem] border-2 border-dashed border-gray-200">
                <div className="flex items-center justify-between mb-10 flex-wrap gap-4 text-right">
                  <div className="flex items-center gap-4">
                    ...
                    <h3 className="text-2xl font-black text-gray-800">مراجعة تقرير Junior UX Designer</h3>
                  </div>
                  ...
                  <div className="mb-6 p-4 bg-emerald-50 rounded-2xl border border-emerald-100 flex justify-between items-center no-pdf">
                     ...
                     <div className="flex flex-col items-end">
                       ...
                       <span className={`text-[10px] font-bold ${calculateTotalScore() >= 75 ? 'text-emerald-500' : calculateTotalScore() >= 60 ? 'text-amber-500' : 'text-rose-500'}`}>
                         {calculateTotalScore() >= 75 ? 'Ready for Junior UX Designer' : calculateTotalScore() >= 60 ? 'Needs More Experience' : 'Not Ready Yet'}
                       </span>
                     </div>
                  </div>
                  <textarea 
          ...
          {/* Sidebar: Junior UX Specific Question Bank */}
          {showQuestions && (
            <div className="w-80 bg-gray-50 border-r border-gray-100 overflow-y-auto p-6 animate-in slide-in-from-left duration-300 text-right">
              <div className="flex items-center justify-between mb-8">
                ...
                <h4 className="font-black text-gray-900 flex items-center gap-2">
                   <Palette className="w-4 h-4 text-accent" /> بنك الأسئلة للمحاور
                </h4>
              </div>

              <div className="space-y-8">
                <section>
                  <h5 className="text-[10px] font-black text-violet-600 uppercase tracking-widest mb-3 border-b border-violet-100 pb-1 flex items-center gap-2">
                    <Brain className="w-3 h-3" /> UX Thinking
                  </h5>
                  <ul className="space-y-3 text-xs font-medium text-gray-600 list-disc list-inside leading-relaxed">
                    <li>Walk me through solving a UX problem from scratch.</li>
                    <li>How do you decide between two solutions?</li>
                    <li>Understanding trade-offs (User vs Business).</li>
                  </ul>
                </section>

                <section>
                  <h5 className="text-[10px] font-black text-emerald-600 uppercase tracking-widest mb-3 border-b border-emerald-100 pb-1 flex items-center gap-2">
                     <Search className="w-3 h-3" /> User Research
                  </h5>
                  <ul className="space-y-3 text-xs font-medium text-gray-600 list-disc list-inside leading-relaxed">
                    <li>How do you validate your design assumptions?</li>
                    <li>How would you improve an existing product?</li>
                    <li>Creating personas based on real insights.</li>
                  </ul>
                </section>
                ...
                <section>
                  <h5 className="text-[10px] font-black text-blue-600 uppercase tracking-widest mb-3 border-b border-blue-100 pb-1 flex items-center gap-2">
                    <PenTool className="w-3 h-3" /> Flows & IA
                  </h5>
                  <p className="text-[10px] text-gray-400 mb-2 italic">Task: Design a checkout flow.</p>
                  <ul className="space-y-3 text-xs font-medium text-gray-600 list-disc list-inside leading-relaxed">
                    <li>How do you handle flow errors?</li>
                    <li>What happens if user abandons?</li>
                    <li>Consistency across multiple screens.</li>
                  </ul>
                </section>
                ...
                <section>
                  <h5 className="text-[10px] font-black text-rose-500 uppercase tracking-widest mb-3 border-b border-rose-100 pb-1 flex items-center gap-2">
                    <GitMerge className="w-3 h-3" /> Tools & Handoff
                  </h5>
                  <ul className="space-y-3 text-xs font-medium text-gray-600 list-disc list-inside leading-relaxed">
                    <li>Figma file structure for developers?</li>
                    <li>Aligning with technical constraints.</li>
                    <li>Interactive prototyping detail.</li>
                  </ul>
                </section>
                
                <div className="bg-violet-50 p-4 rounded-2xl border border-violet-100">
                  <h6 className="text-[10px] font-black text-violet-900 flex items-center gap-2 mb-1">
                    <Zap className="w-3 h-3" /> نصيحة للمحاور
                  </h6>
                  <p className="text-[10px] leading-relaxed text-violet-700 italic">
                    Junior UX = Execution + Reasoning.
                    قيم ليس فقط ما صممه، ولكن "لماذا" وكيف قام بالتطوير بناءً على الملاحظات.
                  </p>
                </div>
              </div>
            </div>
...
export default JuniorUXEvaluation;
