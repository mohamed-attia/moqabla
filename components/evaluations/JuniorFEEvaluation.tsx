...
import { JUNIOR_FE_EVALUATION_TEMPLATE } from '../../lib/evaluation-templates';
...
const JuniorFEEvaluation: React.FC<Props> = ({ registration, onComplete, onCancel, interviewerName }) => {
  ...
  const [sections, setSections] = useState<EvaluationSection[]>(JSON.parse(JSON.stringify(JUNIOR_FE_EVALUATION_TEMPLATE)));

  const scoreLabels = ['Basic Awareness', 'Competent', 'Developing+', 'Consistent', 'Strong Junior'];
  ...
  const handleAiSuggest = async (sectionIdx: number, itemIdx: number) => {
    ...
    try {
      const suggestion = await AIAgent.suggestNote(
          item.skill, 
          item.score, 
          'Junior Frontend Engineer (React/Angular emphasis)'
      );
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-6">
            <div className="flex items-center gap-5">
              ...
              <div className="text-right">
                <h2 className="text-xl md:text-2xl font-black">تقييم جونيور: {registration.fullName}</h2>
                <div className="flex items-center gap-2 text-accent/90 text-sm font-bold mt-1">
                  <Award className="w-4 h-4" />
                  <span>Junior Frontend Assessment Flow</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gray-50 p-6 md:p-12 rounded-[3rem] border-2 border-dashed border-gray-200">
              <div className="flex items-center justify-between mb-10 flex-wrap gap-4">
                <div className="flex items-center gap-4">
                  ...
                  <h3 className="text-2xl font-black text-gray-800">مراجعة تقرير Frontend Junior</h3>
                </div>
...
export default JuniorFEEvaluation;
