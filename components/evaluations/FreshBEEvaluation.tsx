...
import { 
  ...
  X, Award, Zap, AlertTriangle, Info, Coffee, Box
} from 'lucide-react';
import Button from '../Button';
import { AIAgent } from '../../lib/ai-agent';
import { FRESH_BE_EVALUATION_TEMPLATE } from '../../lib/evaluation-templates';
...
const FreshBEEvaluation: React.FC<Props> = ({ registration, onComplete, onCancel, interviewerName }) => {
  ...
  const [sections, setSections] = useState<EvaluationSection[]>(JSON.parse(JSON.stringify(FRESH_BE_EVALUATION_TEMPLATE)));
  ...
  const handleAiSuggest = async (sectionIdx: number, itemIdx: number) => {
    ...
    try {
      const suggestion = await AIAgent.suggestNote(
          item.skill, 
          item.score, 
          'Fresh Backend Engineer (Java / .NET emphasis)'
      );
    ...
    try {
      const report = await AIAgent.generateFinalReport(
        ...
        { 
          ...
          decision: totalScore >= 70 ? 'Ready for Fresh Backend Developer' : totalScore >= 55 ? 'Almost Ready' : 'Not Ready Yet'
        }
      );
    ...
    <div className="fixed inset-0 z-[100] bg-slate-900/90 backdrop-blur-xl flex items-center justify-center p-0 md:p-4 overflow-y-auto">
      <div className="bg-white w-full max-w-6xl md:h-[90vh] md:rounded-[3rem] shadow-2xl flex flex-col animate-in zoom-in-95 duration-300 overflow-hidden relative">
        ...
        <button 
          ...
          className="absolute left-6 top-6 z-[110] bg-white/10 hover:bg-white/20 backdrop-blur-md text-white p-3 rounded-2xl border border-white/20 transition-all flex items-center gap-2 group"
        >
          <Info className="w-5 h-5 group-hover:rotate-12 transition-transform" />
          <span className="text-xs font-black">أسئلة مقترحة</span>
        </button>
            ...
            <div className="flex items-center gap-5">
              ...
              <div className="text-right">
                ...
                <div className="flex items-center gap-2 text-accent/90 text-sm font-bold mt-1">
                  ...
                  <span>Fresh Backend Assessment Flow</span>
                </div>
              </div>
            </div>
        ...
        <div className="flex flex-grow overflow-hidden">
                    ...
                    <div key={i} className="animate-in fade-in slide-in-from-bottom-4" style={{ animationDelay: `${i * 100}ms` }}>
                      <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-6">
                        <div className="space-y-1 flex-grow">
                          ...
                          <p className="text-xs text-gray-400 mr-9">قيم مهارات المرشح في {item.skill} لمستوى Fresh.</p>
                        </div>
              ...
              <div className="bg-gray-50 p-6 md:p-12 rounded-[3rem] border-2 border-dashed border-gray-200">
                <div className="flex items-center justify-between mb-10 flex-wrap gap-4 text-right">
                  <div className="flex items-center gap-4">
                    ...
                    <h3 className="text-2xl font-black text-gray-800">مراجعة تقرير Backend Fresh</h3>
                  </div>
                  ...
                  <div className="mb-6 p-4 bg-emerald-50 rounded-2xl border border-emerald-100 flex justify-between items-center no-pdf">
                     ...
                     <div className="flex flex-col items-end">
                       ...
                       <span className={`text-[10px] font-bold ${calculateTotalScore() >= 70 ? 'text-emerald-500' : calculateTotalScore() >= 55 ? 'text-amber-500' : 'text-rose-500'}`}>
                         {calculateTotalScore() >= 70 ? 'Ready for Fresh Backend' : calculateTotalScore() >= 55 ? 'Almost Ready' : 'Not Ready Yet'}
                       </span>
                     </div>
                  </div>
                  <textarea 
          ...
          {/* Sidebar: Interview Questions Map */}
          {showQuestions && (
            <div className="w-80 bg-gray-50 border-r border-gray-100 overflow-y-auto p-6 animate-in slide-in-from-left duration-300 text-right">
              <div className="flex items-center justify-between mb-8">
                ...
                <h4 className="font-black text-gray-900 flex items-center gap-2">
                   <Coffee className="w-4 h-4 text-accent" /> بنك الأسئلة
                </h4>
              </div>

              <div className="space-y-8">
                <section>
                  <h5 className="text-[10px] font-black text-accent uppercase tracking-widest mb-3 border-b border-accent/10 pb-1">1. Programming (Java/.NET)</h5>
                  <ul className="space-y-3 text-xs font-medium text-gray-600 list-disc list-inside">
                    <li>Explain OOP concepts (Encapsulation, Inheritance).</li>
                    <li>Difference between primitive and reference types.</li>
                    <li>What is JVM? (Java) vs CLR (.NET).</li>
                    <li>Interface vs Abstract class (.NET).</li>
                  </ul>
                </section>
                ...
                <section>
                  ...
                  <ul className="space-y-3 text-xs font-medium text-gray-600 list-disc list-inside">
                    <li>How would you validate user input?</li>
                    <li>Find the max value in a list of numbers.</li>
                    <li>How do you handle null pointer exceptions?</li>
                  </ul>
                </section>

                <section>
                  <h5 className="text-[10px] font-black text-accent uppercase tracking-widest mb-3 border-b border-accent/10 pb-1">3. Data Structures</h5>
                  <ul className="space-y-3 text-xs font-medium text-gray-600 list-disc list-inside">
                    <li>List vs Map: When to use which?</li>
                    <li>Difference between ArrayList and LinkedList.</li>
                    <li>How to loop through a map and print values?</li>
                  </ul>
                </section>

                <section>
                  <h5 className="text-[10px] font-black text-accent uppercase tracking-widest mb-3 border-b border-accent/10 pb-1">4. Backend & API</h5>
                  <ul className="space-y-3 text-xs font-medium text-gray-600 list-disc list-inside">
                    <li>What is a REST API?</li>
                    <li>GET vs POST: Differences and use cases.</li>
                    <li>HTTP 404 vs 500 status codes.</li>
                    <li>How does backend talk to a DB?</li>
                  </ul>
                </section>
                ...
                <div className="bg-amber-50 p-4 rounded-2xl border border-amber-100">
                  ...
                  <p className="text-[10px] leading-relaxed text-amber-700 italic">
                    Fresh Level != Framework Expert.
                    ركز على التفكير، الأساسيات، والرغبة في التعلم.
                  </p>
                </div>
              </div>
            </div>
...
export default FreshBEEvaluation;
